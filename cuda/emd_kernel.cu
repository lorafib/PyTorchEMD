...
at::Tensor ApproxMatchForward(
  ...
  TORCH_CHECK(xyz1.size(2) == 3, (std::string("is not 3: ")+std::to_string(xyz1.size(2))).c_str());
  TORCH_CHECK(xyz2.size(2) == 3, (std::string("is not 3: ")+std::to_string(xyz2.size(2))).c_str());
  CHECK_INPUT(xyz1);
  CHECK_INPUT(xyz2);
